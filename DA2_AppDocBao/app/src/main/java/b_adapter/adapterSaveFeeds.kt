...
import a_screen.ScreenDetail_2
...
class adapterSaveFeeds(val context: Context, var list: List<ItemFeed>): RecyclerView.Adapter<adapterSaveFeeds.viewHolder>() {
    inner class viewHolder(var binding: ItemNewfeedBinding) : RecyclerView.ViewHolder(binding.root)

    private var originalList: List<ItemFeed> = list // Lưu trữ danh sách gốc
    ...
    override fun onBindViewHolder(holder: viewHolder, position: Int) {
        if (list.isNotEmpty()) { // kiểm tra xem danh s
            holder.binding.txtTitle.text = list[position].title.toString()

            val showdate = formatDateToVietnamese(list[position].pubDate.toString())
            holder.binding.txtDate.text = showdate
            Picasso.get().load(extractImageUrl(list[position].description))
                .into(holder.binding.imgNewFeed)
            holder.itemView.setOnClickListener {// xét sự kiện
                val intent = Intent(context, ScreenDetail_2::class.java)
                intent.putExtra("linkImage", extractImageUrl(list[position].description))
                intent.putExtra("link", list[position].link)
                intent.putExtra("date", showdate)
                intent.putExtra("title", list[position].title)
                intent.putExtra("creator", list[position].creator)
