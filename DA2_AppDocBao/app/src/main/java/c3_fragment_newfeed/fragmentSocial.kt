 ...
 */
class fragmentSocial : Fragment(R.layout.fragment_social) {
    ...
    val url = "https://dantri.com.vn/rss/xa-hoi.rss"
    ...
    private fun show_newfeeds() {
       ...
       /*
       viewLifecycleOwner.lifecycleScope :được liên kết với vòng đời của View trong Fragment hoặc Activity.
       Điều này có nghĩa là tất cả các coroutines được khởi chạy trong scope này sẽ tự động bị hủy khi View bị destroy.
       Nên sử dụng viewLifeCycleOwner này để thực hiện cập nhật từ mạng vì nó sẽ tự động hủy Coroutine khi t chuyển hoặc thoát khỏi Activity
        ...
        */
        {
                if (isAdded) {
                    ...
                    /*
                     if (isAdded) kiểm tra xem Fragment hiện tại có được gắn vào một Activity hay không.
                      Điều này quan trọng để đảm bảo rằng bạn không cố gắng truy cập Context (như requireContext())
                      khi Fragment chưa được gắn, tránh lỗi IllegalStateException.
                     */
            ...
            withContext(Dispatchers.Main) {
                if (isAdded) {
                    if (rssFeed != null) {
                        ...
                        if (list != null) {
                            binding.recycleAllfeeds.layoutManager = LinearLayoutManager(requireContext())
